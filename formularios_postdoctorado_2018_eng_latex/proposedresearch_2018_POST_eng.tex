...
The SNN approach allows to build more robust distance measures for high dimensional data and the density-based scheme such as the one used by \citep{ESK03} allows to find representative points in regions with a non-spheric shape.

The proposed framework uses a Master/Slave model of computation in which a master node partitions the dataset and then each slave or worker node processes its assigned chunk of data in order to identify points that represents each group (\textit{Datset partitioning} and \textit{Local model generation} stages in Figure
